...
public:
    ...
    EmitLoadOfMemberFunctionPointer(CodeGenFunction &CGF,
      ...
      const FunctionProtoType *FPT = MPT->getPointeeType()->getAs<FunctionProtoType>();
      auto *RD = cast<CXXRecordDecl>(MPT->getClass()->castAs<RecordType>()->getDecl());

      llvm::FunctionType *FTy = CGM.getTypes().GetFunctionType(CGM.getTypes().arrangeCXXMethodType(RD, FPT, /*FD=*/nullptr));
      ...
      llvm::Value *NonVirtualFn = Builder.CreateIntToPtr(FnAsInt, FTy->getPointerTo(), "memptr.nonvirtualfn");

      // We're done.
      CGCallee Callee(FPT, NonVirtualFn);
      return Callee;
    }
    ...
    llvm::Constant *EmitMemberFunctionPointer(const CXXMethodDecl *MD) override {
      ...
      if (MD->isVirtual() && !excludeFromAbi(MD)) {
        ...
        uint64_t VtableIndex = CGM.getItaniumVTableContext().getMethodVTableIndex(MD);
        std::string dispatcherName = "__novt." + mangleClass(RD) + "." + std::to_string(VtableIndex) + "." + mangleFunctionNameForInheritance(MD);
        auto &M = (llvm::Module &) CGM.getModule();
        llvm::Value *dispatcher = M.getFunction(dispatcherName);
        if (!dispatcher) {
          dispatcherName = "__novt_placeholder." + mangleClass(RD) + "." + std::to_string(VtableIndex) + "." + mangleFunctionNameForInheritance(MD);
    ...
    llvm::Value *GetVirtualBaseClassOffset(CodeGenFunction &CGF, Address This,
      ...
      CharUnits VBaseOffsetOffset = CGM.getItaniumVTableContext().getVirtualBaseOffsetOffset(ClassDecl, BaseClassDecl);
      unsigned SlotSize = CGM.getDataLayout().getPointerSize(0);
      auto funcname = "__novt_vbase_offset." + mangleClass(ClassDecl) + "." + mangleClass(BaseClassDecl) + "." +
                      std::to_string(VBaseOffsetOffset.getQuantity() / SlotSize);
      /*
    ...
    CGCallee getVirtualFunctionPointer(CodeGenFunction &CGF, GlobalDecl GD,
      ...
      std::string dispatcherName = "__novt." + mangleClass(MethodDecl->getParent()) + "." + std::to_string(VTableIndex)
    ...
    llvm::MDTuple *getMetaForMethod(CXXMethodDecl *method, CXXDtorType dtor, CXXDtorType dtor2) {
      ...
      int64_t VTableIndex = -1;
      if (isa<CXXDestructorDecl>(method)) {
        if (dtor == Dtor_Deleting || dtor == Dtor_Complete) {
          VTableIndex = CGM.getItaniumVTableContext().getMethodVTableIndex(GlobalDecl(dyn_cast<CXXDestructorDecl>(method), dtor));
        }
      } else {
        VTableIndex = CGM.getItaniumVTableContext().getMethodVTableIndex(GlobalDecl(method));
      }
      std::vector<llvm::Metadata *> data{llvm::MDString::get(C, mangleFunctionNameForInheritance(method, dtor)),
                                         llvm::MDString::get(C, symbol),
                                         llvm::MDString::get(C, std::to_string(VTableIndex))};
      auto func = CGM.getModule().getFunction(symbol);
      if (!func && StringRef(symbol).endswith("D1Ev")) {
        std::string symbol2 = symbol;
        symbol2[symbol2.size() - 3] = '2';
        func = CGM.getModule().getFunction(symbol2);
      } else if (!func && StringRef(symbol).endswith("D2Ev")) {
        std::string symbol2 = symbol;
        symbol2[symbol2.size() - 3] = '1';
        func = CGM.getModule().getFunction(symbol2);
      }
      if (func) {
