...
void ToolChain::LinkInstrumentationLibrary(ArgStringList &CmdArgs) const {
  ...
  bool isStatic = Args.getLastArgNoClaim(options::OPT_static) != nullptr || Args.getLastArgNoClaim(options::OPT_static_pie) != nullptr;
  bool isShared = Args.getLastArgNoClaim(options::OPT_shared) != nullptr;

  // Link JIT in libc and static applications
  if (typegraph::Settings.enabled && typegraph::Settings.enforce_jit) {
    if (isStatic || typegraph::Settings.enforce_jit_link) {
      auto *Arg = Args.getLastArgNoClaim(options::OPT__sysroot_EQ);
      auto *File = new std::string(Arg ? Arg->getValue() : "/usr");
      File->append("/lib/patchpoline_jit.o");
      CmdArgs.push_back(File->c_str());
    }
  }
  // Link PLT rewriter to everything that might have a PLT section
  if (typegraph::Settings.enabled && typegraph::Settings.dynamic_linking) {
    if (!isStatic && Args.getLastArgNoClaim(options::OPT_fno_plt) == nullptr) {
      auto *Arg = Args.getLastArgNoClaim(options::OPT__sysroot_EQ);
      auto *File = new std::string(Arg ? Arg->getValue() : "/usr");
      File->append("/lib/patchpoline_plt.o");
      CmdArgs.push_back(File->c_str());
      // also enable RELRO (which the PLT rewriter assumes)
      CmdArgs.push_back("-z");
