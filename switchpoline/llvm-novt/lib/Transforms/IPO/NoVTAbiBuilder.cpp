...
namespace {
    ...
    std::map<const std::string, uint32_t> defaultVtableIDs = {
            {"_ZTVN10__cxxabiv116__enum_type_infoE", 0x13370000},
            {"_ZTVN10__cxxabiv117__array_type_infoE", 0x13370001},
            {"_ZTVN10__cxxabiv117__class_type_infoE", 0x13370002},
            {"_ZTVN10__cxxabiv119__pointer_type_infoE", 0x13370003},
            {"_ZTVN10__cxxabiv120__function_type_infoE", 0x13370004},
            {"_ZTVN10__cxxabiv120__si_class_type_infoE", 0x13370005},
            {"_ZTVN10__cxxabiv121__vmi_class_type_infoE", 0x13370006},
            {"_ZTVN10__cxxabiv123__fundamental_type_infoE", 0x13370007},
            {"_ZTVN10__cxxabiv129__pointer_to_member_type_infoE", 0x13370008},
    };


    bool isStandardLibrary(const StringRef &S) {
      return S.startswith("St") || S.startswith("NSt") || S.find("__cxxabiv");
    }
    ...
    public:
        ...
        std::map<std::pair<std::string, int>, long> vtableOffsets; // (function name, thunk offset) => offset of its vtable entry
        ...
        bool isExternal = false; // can be defined or extended by classes in other libraries
        ...
        void setVtable(const std::string &symbol, const std::string &idmap, Value *table) {
          vtableSymbol = symbol;
          vtable = dyn_cast_or_null<GlobalVariable>(table);
        ...
        bool isVirtual() {
          if (!definitions.empty() || !vbases.empty() || hasVirtualFunctions)
            return true;
          for (auto &base: bases) {
        ...
        void print(llvm::raw_ostream &S) {
          S << "class " << name << " : ";
          for (auto b: bases) S << b.cls->name << "(" << b.offset << ") ";
          for (auto b: vbases) S << "virtual " << b.cls->name << "(" << b.offset << ") ";
          S << "{\n";
          for (const auto &it: definitions)
            S << "    " << it.first.first << " +" << it.first.second << "\n";
          // S << "    " << it.first.first << " +" << it.first.second << "  (symbol " << it.second->getName() << ")\n";
          if (!vtableSymbol.empty()) {
            S << "    vtable " << vtableSymbol << "\n";
            S << "    vtable index map:  ";
            for (const auto &it: vtableIdMap) S << it.first << ":" << it.second << " ";
            S << "\n";
          }
    ...
    public:
        ...
        void addCase(DefClassIdent *ident, BasicBlock *block) {
          ...
          if (Settings.protect_spectre_v1) {
            auto TT = StringRef(block->getParent()->getParent()->getTargetTriple());
        ...
        int getIdentSizeIgnoringStdlib() {
          if (identSizeIgnoringStdlib >= 0)
            return identSizeIgnoringStdlib;
          identSizeIgnoringStdlib = 0;
          for (auto *ident: idents) {
            if (ident->cls->name.substr(0, 2) == "St")
              continue;
            if (ident->cls->name.substr(0, 11) == "N9libunwind")
              continue;
            if (ident->cls->name.find("itanium_demangle") != std::string::npos)
              continue;
            if (ident->cls->name.find("__cxxabiv") != std::string::npos)
              continue;
            identSizeIgnoringStdlib++;
          }
          return identSizeIgnoringStdlib;
        }
    };
    ...
    class AbiBuilder {
        ...
        long jit_min_id = 0xffffffff;
        long jit_max_id = 0xffffffff;
        bool hasDefaultVtableIDsAssigned = false;
    public:
        ...
        DefClass *getClass(const std::string &name, bool isNotInitialParsing = false) {
          auto it = classes.find(name);
          if (it != classes.end())
            return it->second.get();
          DefClass *Cls = classes.insert({name, std::make_unique<DefClass>(name)}).first->second.get();
          if (isNotInitialParsing && Settings.enforce_jit && (Settings.novt_jit_all || isStandardLibrary(name))) {
            Cls->isExternal = true;
          }
          return Cls;
        }
            ...
            if (md.getName().endswith(".bases")) {
              ...
              for (unsigned i = 0; i < md.getNumOperands(); i++) {
                if (md.getOperand(i)->getNumOperands() > 0) {
                  ...
                  if (!isa<MDString>(md.getOperand(i)->getOperand(2))) {
                    llvm::errs() << "IN " << md.getName() << " " << i << " => " << *md.getOperand(i) << "\n";
                  }
                  auto vtableIndex = std::stol(cast<MDString>(md.getOperand(i)->getOperand(2))->getString());
                  auto funcRef = dyn_cast_or_null<ValueAsMetadata>(md.getOperand(i)->getOperand(3));
                  if (funcRef && funcRef->getValue()) {
                    auto f = dyn_cast<Function>(funcRef->getValue());
                    if (f) {
                      cls->definitions.insert({{funcname, 0}, f});
                      cls->vtableOffsets.insert({{funcname, 0}, vtableIndex});
                    }
                  } else {
                  ...
                  int thunkOffset = 0;
                  for (unsigned int j = 4; j < md.getOperand(i)->getNumOperands(); j++) {
                    ...
                    if (thunkspec) {
                      ...
                      if (thunkFunction && thunkFunction->getValue()) {
                        ...
                        if (f) {
                          ...
                          cls->vtableOffsets.insert({{funcname, thunkOffset}, vtableIndex});
                        }
                      } else {
              ...
              for (unsigned int i = 0; i < md.getNumOperands(); i++) {
                ...
                if (vtable) {
                  ...
                  cls->setVtable(vtable->getValue()->getName(), idmap, vtable->getValue());
                  break;
                } else if (op0) {
                  cls->setVtable(cast<MDString>(op0)->getString(), idmap, nullptr);
                }
              }
        ...
        void printToFile() {
          std::error_code EC;
          llvm::raw_fd_ostream S("classes.txt", EC);
          for (auto &it: classes) {
            it.second->print(S);
          }
          S.close();
        ...
        void selectExternalClasses() {
          if (!Settings.enforce_jit)
            return;
          if (Settings.novt_jit_all) {
            for (auto &it: classes) {
              it.second->isExternal = true;
            }
          } else {
            std::queue<DefClass *> q;
            for (auto &it: classes) {
              if (isStandardLibrary(it.first)) {
                q.push(it.second.get());
              }
            }
            while (!q.empty()) {
              DefClass *c = q.front();
              ...
              if (c->isExternal)
                continue;
              c->isExternal = true;
              for (const auto &x: c->children) {
                q.push(x.cls);
              }
              for (const auto &x: c->vchildren) {
                q.push(x.cls);
              }
            }
          }
        ...
        void buildClassNumbers() {
          uint32_t i = 0;
          for (auto &it: classes) {
            // Check if this class has a predetermined ID
            uint32_t fixedId = 0;
            if (Settings.enforce_jit) {
              auto defaultIdIt = defaultVtableIDs.find("_ZTV" + it.second->name);
              if (defaultIdIt != defaultVtableIDs.end()) {
                fixedId = defaultIdIt->second;
                hasDefaultVtableIDsAssigned = true;
              }
            }
        ...
        public:
            ...
            Value *buildIdentLoad(BasicBlock *block) {
              if (loadedIdent == nullptr) {
                ...
                loadedIdent = builder.CreateAlignedLoad(ptrToVtableCasted, M.getDataLayout().getPointerPrefAlignment());
              }
              return loadedIdent;
            }
        };
        ...
        void buildFunction(T &t, DefClass *staticType) {
          ...
          t.defaultCase = BasicBlock::Create(C, "defaultCase", t.function);
          ...
          t.switchInst = builder.CreateSwitch(vtableIndex, t.defaultCase);
          ...
          for (auto &ident: idents) {
            if (!ident->abstract) {
              for (auto identUser: ident->users) {
                auto num = ConstantInt::get(IntPtr, ident->getNumber(), false);
        ...
        /**
         * Some vtables (for auto-generated RTTI) are imported symbols referenced from global data.
         * We have globally defined IDs for them (synchronized between their usage in programs and libcxxabi).
         * Looks like this in LLVM:
         * @_ZTIP15CustomException = internal constant { i8*, i8*, i32, i8* } { i8* bitcast (i8** getelementptr inbounds (i8*, i8** @_ZTVN10__cxxabiv119__pointer_type_infoE, i64 2) to i8*), i8* getelementptr inbounds ([19 x i8], [19 x i8]* @_ZTSP15CustomException, i32 0, i32 0), i32 0, i8* bitcast ({ i8*, i8* }* @_ZTI15CustomException to i8*) }, comdat, align 8
         * @_ZTVN10__cxxabiv119__pointer_type_infoE = external global i8*
         */
        void patchDefaultVtables() {
          if (!Settings.enforce_jit)
            return;
          // replace all automatic-inserted occurences of these vtables (usually in type infos of global objects).
          // do not replace defining occurences (when compiling libcxxabi).
          for (auto &it: defaultVtableIDs) {
            auto *GV = M.getGlobalVariable(it.first);
            if (GV && GV->getLinkage() == llvm::GlobalValue::ExternalLinkage && !GV->hasInitializer() && GV->getValueType() == Type::getInt8PtrTy(C)) {
              std::vector<llvm::User *> users;
              for (auto *user: GV->users()) {
                if (auto *BC = dyn_cast<GEPOperator>(user)) {
                  users.push_back(BC);
                }
              }
              for (auto *user: users) {
                typesafeReplaceAllUsesWith(user, ConstantInt::get(IntPtr, it.second, false));
              }
            }
          }
        }
        ...
        int setDispatcherArgumentParameters(Function &f, StringRef &classname, StringRef &function) {
          ...
          collectCallTargets(callTargets, getClass(classname, true), function, 0);
          if (callTargets.empty()) {
            if (getClass(classname, true)->isExternal) {
              // for external classes with no implementations we must guess the "this" argument.
        ...
        void defineVirtualDispatcher(Function *f) {
          ...
          auto idx2 = f->getName().find_first_of('.', idx + 1);
          long vtableIndex = std::stol(f->getName().substr(idx + 1, idx2 - idx - 1));
          auto function = f->getName().substr(idx2 + 1);
          dout << "define: " << f->getName() << " class " << classname << " func " << function << " used "
               << f->getNumUses() << "\n";
          if (classes.count(classname) == 0) {
            if (Settings.enforce_jit && (Settings.novt_jit_all || isStandardLibrary(classname))) {
              getClass(classname, true);
            } else {
          ...
          if (f->isVarArg()) {
            f = pullVarArgParametersFromCall(f);
          }
          ...
          // the vtable-index is only necessary for JIT stuff. Disable if no JIT is required.
          if (!getClass(classname, true)->isExternal) {
            vtableIndex = -1;
          }
          ...
          struct DispatcherBuilder : public FunctionBuilder<std::pair<Function *, int>> {
              ...
              explicit DispatcherBuilder(Function *function, long VtableIndex)
                : FunctionBuilder(DISPATCHER, function), VtableIndex(VtableIndex) {
                defaultIsImportant = (VtableIndex != -1);
              }
              ...
              BasicBlock *createCaseBlock(DefClassIdent *ident, bool isRoot, BasicBlock *parentBlock) {
                ...
                if (func) {
                  ...
                  for (size_t i = 0; i < function->arg_size() && (i < defFuncType->getNumParams() || defFuncType->isVarArg()); i++) {
                    ...
                    } else if (i >= defFuncType->getNumParams() || arg->getType() == defFuncType->getFunctionParamType(i)) {
              ...
              void buildDefaultCase(BasicBlock *defaultCaseBlock) {
                IRBuilder<> builder(defaultCaseBlock);
                // Generate JIT call if necessary
                if (VtableIndex != -1) {
                  auto Dispatcher = M.getOrInsertFunction("__novt_default_handler", FunctionType::get(Type::getVoidTy(M.getContext()), false));
                  // move call target to r11 (x64) / x13(aarch64)
                  const char *Constraints;
                  auto TT = StringRef(M.getTargetTriple());
                  if (TT.startswith("x86_64-")) {
                    Constraints = "{r11},{r10},~{dirflag},~{fpsr},~{flags}";
                  } else if (TT.startswith("aarch64-")) {
                    Constraints = "{x13},{x12},~{x14},~{x15}";
                  ...
                  auto *Asm = InlineAsm::get(
                          FunctionType::get(Type::getVoidTy(M.getContext()), {loadedIdent->getType(), IntPtr()}, false),
                          "", Constraints, true
                  );
                  builder.CreateCall(Asm, {loadedIdent, ConstantInt::get(IntPtr(), VtableIndex, false)});

                  std::vector<Value *> Arguments;
                  for (auto &A: function->args()) {
                    Arguments.push_back(&A);
                  }
                  auto *Call = builder.CreateCall(createCast(builder, Dispatcher.getCallee(), function->getFunctionType()->getPointerTo()), Arguments);
                  // copy over parameter attributes
                  for (size_t I = 0; I < function->arg_size(); I++) {
                    if (function->hasParamAttribute(I, Attribute::AttrKind::ByVal))
                      Call->addParamAttr(I, Attribute::AttrKind::ByVal);
                  ...
                  // non-jit default case - terminate application (CFI style)
                  if (BUILD_TRAPS) {
                    builder.CreateIntrinsic(Intrinsic::trap, {}, {});
                  }
                  builder.CreateUnreachable();
          ...
          DispatcherBuilder builder(f, vtableIndex);
          builder.thisArgNumber = thisArgNumber;
          builder.functionName = function;
          buildFunction(builder, getClass(classname, true));
        ...
        void defineVirtualDispatcherPlaceholder(GlobalVariable &g) {
          ...
          if (!func) {
            ...
            auto idx2 = functionName.find_first_of('.', idx + 1);
            auto vtableIndex = functionName.substr(idx + 1, idx2 - idx - 1);
            auto function = functionName.substr(idx2 + 1);
        ...
        void defineRttiGetter(Function &f) {
          ...
          assert(classes.count(classname.str()) > 0);
        ...
        void defineDynamicCast(Function &f) {
          ...
          auto srcClass = getClass(classNames.substr(0, classNamesSplit), true);
          auto dstClass = getClass(classNames.substr(classNamesSplit + 1), true);
        ...
        void defineDynamicCastToVoid(Function &f) {
          auto srcClass = getClass(f.getName().substr(25), true);
          assert(srcClass != nullptr && "srcClass is null");
        ...
        void defineVbaseOffsetGetter(Function &f) {
          ...
          auto classNames = f.getName().substr(20);
          auto classNamesSplit = classNames.find('.');
          auto classNamesSplit2 = classNames.find('.', classNamesSplit + 1);
          auto srcClass = getClass(classNames.substr(0, classNamesSplit), true);
          auto dstClass = getClass(classNames.substr(classNamesSplit + 1, classNamesSplit2 - classNamesSplit - 1), true);
          auto vtableOffset = std::stol(classNames.substr(classNamesSplit2 + 1));
          assert(srcClass != nullptr && "srcClass is null");
          assert(dstClass != nullptr && "dstClass is null");
          ...
          struct VBaseOffsetBuilder : public FunctionBuilder<long> {
              ...
              explicit VBaseOffsetBuilder(Function *function, DefClass *dst, long VtableOffset)
                      : FunctionBuilder(VBASE_OFFSET, function), dst(dst), VtableOffset(VtableOffset) {
                defaultIsImportant = dst->isExternal;
                if (!BUILD_INSTRUMENTATION) {
              ...
              void buildDefaultCase(BasicBlock *defaultCaseBlock) {
                ...
                if (dst->isExternal) {
                  ...
                  builder.CreateCall(Asm, {loadedIdent, ConstantInt::get(IntPtr(), VtableOffset, false)});
          ...
          VBaseOffsetBuilder builder(&f, dstClass, vtableOffset);
          buildFunction(builder, srcClass);

          count_vbase_offset_getter++;
        }

        void defineVirtualDispatchDefaultHandler() {
          if (!Settings.enforce_jit)
            return;
          bool hasExternal = false;
          for (auto &Ident: idents) {
            if (!Ident->removed && !Ident->abstract && Ident->cls->hasVirtualFunctions && Ident->cls->isExternal && Ident->cls->vtable != nullptr) {
              hasExternal = true;
              break;
            }
          }
          ...
          // Create a function that can dispatch ALL external virtual methods
          struct DefaultDispatcherBuilder : public FunctionBuilder<std::pair<Function *, int>> {
              explicit DefaultDispatcherBuilder(Function *function) : FunctionBuilder(DISPATCHER, function) {}

              Value *loadedVtableIndex = nullptr;
              ...
              Value *buildIdentLoad(BasicBlock *block) {
                if (loadedIdent == nullptr) {
                  ...
                  auto *Asm = InlineAsm::get(FunctionType::get(StructType::get(IntPtr(), Int32()), {}, false),"", Constraints, true);
                  Value *V = builder.CreateCall(Asm);
                  loadedIdent = builder.CreateExtractValue(V, {0});
                  loadedVtableIndex = builder.CreateExtractValue(V, {1});
                }
                return loadedIdent;
              }

              BasicBlock *createCaseBlock(DefClassIdent *ident, bool isRoot, BasicBlock *parentBlock) {
                auto *Vtables = cast<ConstantStruct>(ident->cls->vtable->getInitializer());
                auto *Vtable = cast<ConstantArray>(Vtables->getOperand(ident->vtable_index));
                // find the beginning of the vtable
                unsigned Offset = 0;
                while (Offset < Vtable->getNumOperands()) {
                  if (auto *BC = dyn_cast<BitCastOperator>(Vtable->getOperand(Offset))) {
                    if (BC->getOperand(0)->getType()->isPointerTy() && BC->getOperand(0)->getType()->getPointerElementType()->isFunctionTy()) {
                      break;
                    }
                  }
                  Offset++;
                ...
                // Create a switch-case over vtable entries
                std::string cn = getCaseName(ident);
                auto *IdentBlock = BasicBlock::Create(C, cn, function);
                IRBuilder<> Builder(IdentBlock);
                SwitchInst *Switch = Builder.CreateSwitch(loadedVtableIndex, defaultCase);
                ...
                // Create a direct call for each vtable entry
                for (unsigned I = 0; I < Vtable->getNumOperands(); I++) {
                  BasicBlock *BB = BasicBlock::Create(C, cn + "_" + std::to_string(I), function);
                  Switch->addCase(ConstantInt::get(Type::getInt32Ty(C), I - Offset, false), BB);
                  Builder.SetInsertPoint(BB);
                  auto *BC = dyn_cast<BitCastOperator>(Vtable->getOperand(I));
                  if (BC && BC->getOperand(0)->getType()->isPointerTy() && BC->getOperand(0)->getType()->getPointerElementType()->isFunctionTy()) {
                    Value *Result = Builder.CreateCall(Builder.CreateBitCast(Vtable->getOperand(I), function->getType()), Args);
                    Builder.CreateRet(Result);
                  } else {
                    Builder.CreateRet(createCast(Builder, Vtable->getOperand(I), function->getReturnType()));
                  }
                }
          ...
          auto *F = cast<Function>(M.getOrInsertFunction("__novt_all_id_handler", FType).getCallee());

          DefaultDispatcherBuilder builder(F);
          builder.thisArgNumber = -1;
          buildFunction(builder, nullptr);

          for (auto &Ident: idents) {
            if (!Ident->removed && !Ident->abstract && Ident->cls->hasVirtualFunctions && Ident->cls->isExternal && Ident->cls->vtable != nullptr) {
              BasicBlock *block = builder.createCaseBlock(Ident.get(), false, nullptr);
              if (block) {
                builder.visitedIdents.insert(Ident.get());
                builder.swfunc->addCase(Ident.get(), block);
              } else if (builder.swfunc->defaultIsImportant) {
                builder.swfunc->addDefaultCase(Ident.get());
              }
            }
          }
        }

        void registerVirtualDispatchDefaultHandler() {
          auto *HandlerFunction = M.getFunction("__novt_all_id_handler");
          if (!HandlerFunction || jit_min_id == 0xffffffff)
            return;
          // Register it with the JIT on startup
          auto *VoidTy = Type::getVoidTy(M.getContext());
          auto *PtrTy = Type::getInt8PtrTy(M.getContext());
          auto *IntTy = Type::getInt64Ty(M.getContext());
          auto *RegisterFunc = cast<Function>(M.getOrInsertFunction("__novt_all_id_handler_register", FunctionType::get(VoidTy, {}, false)).getCallee());
          auto JITRegisterFunc = M.getOrInsertFunction("__novt_register_handler", FunctionType::get(VoidTy, {PtrTy, IntTy, IntTy}, false));
          // registerfunc calls JITRegisterFunc(HandlerFunc, MinId, MaxId)
          auto *BB = BasicBlock::Create(M.getContext(), "entry", RegisterFunc);
          IRBuilder<> IRB(BB);
          IRB.CreateCall(JITRegisterFunc, {IRB.CreateBitCast(HandlerFunction, PtrTy), ConstantInt::get(IntTy, jit_min_id, false), ConstantInt::get(IntTy, jit_max_id, false)});
          if (hasDefaultVtableIDsAssigned) {
            uint32_t defaultMin = defaultVtableIDs.begin()->second;
            uint32_t defaultMax = defaultMin;
            for (auto &it: defaultVtableIDs) {
              defaultMin = std::min(defaultMin, it.second);
              defaultMax = std::max(defaultMin, it.second);
            }
            IRB.CreateCall(JITRegisterFunc, {IRB.CreateBitCast(HandlerFunction, PtrTy), ConstantInt::get(IntTy, defaultMin, false), ConstantInt::get(IntTy, defaultMax, false)});
          }
          IRB.CreateRetVoid();
          // call registerfunc on startup
          RegisterFunc->setLinkage(Function::InternalLinkage);
          HandlerFunction->setLinkage(Function::InternalLinkage);
        ...
        Function *pullVarArgParametersFromCall(Function *f) {
          // collect all vararg argument types
          unsigned givenArguments = f->arg_size();
          unsigned maxAdditionalParameters = 0;
          unsigned firstInvalidParameter = 0xffffffff;
          for (auto *User: f->users()) {
            if (auto *BC = dyn_cast<BitCastOperator>(User)) {
              User = cast<llvm::User>(BC->getOperand(0));
            }
            if (auto *Call = dyn_cast<CallBase>(User)) {
              if (Call->getNumArgOperands() <= givenArguments)
                continue;
              maxAdditionalParameters = std::max(maxAdditionalParameters, Call->getNumArgOperands() - givenArguments);
              for (unsigned i = givenArguments; i < Call->getNumArgOperands(); i++) {
                auto *T = Call->getArgOperand(i)->getType();
                if (!T->isIntegerTy() && !T->isPointerTy() && !T->isFloatingPointTy()) {
                  firstInvalidParameter = std::min(firstInvalidParameter, i);
                }
              }
            }
          }
          if (maxAdditionalParameters == 0 || firstInvalidParameter == 0)
            return f;

          // create a new type signature
          std::vector<Type *> argTypes;
          for (unsigned i = 0; i < f->getFunctionType()->getNumParams(); i++) {
            argTypes.push_back(f->getFunctionType()->getParamType(i));
          }
          for (unsigned i = 0; i < maxAdditionalParameters && i < firstInvalidParameter; i++) {
            argTypes.push_back(M.getDataLayout().getIntPtrType(C));
          }

          // replace function with a new one
          StringRef name = f->getName();
          f->setName(name + "__old");
          Function *f2 = cast<Function>(M.getOrInsertFunction(name, FunctionType::get(f->getReturnType(), argTypes, true)).getCallee());
          f->replaceAllUsesWith(ConstantExpr::getBitCast(f2, f->getType()));
          return f2;
        }
        ...
        void writeInstrumentationFile() {
          ...
          for (auto &sf: createdFunctions) {
            ...
            int NumCases = 0;
            std::set<BasicBlock *> Blocks;
            for (auto &it: sf->cases) {
              if (it.first->removed || it.first->replacedBy)
                continue;
              NumCases++;
              Blocks.insert(it.second);
            }
            if (NumCases == 0 && sf->removed) {
              // this was a devirtualized call
              NumCases = 1;
              Blocks.insert(nullptr);
            }
            f << type_string << "," << sf->function->getName().str() << "," << NumCases << ","
              << Blocks.size() << "," << sf->function->getNumUses() << "\n";
          }
        ...
        bool run() {
          ...
          // for jit
          defineVirtualDispatchDefaultHandler();
          ...
          patchDefaultVtables();
          buildCases();
          registerVirtualDispatchDefaultHandler();
          printSummary();
          printFunctionStatistics();
          if (BUILD_INSTRUMENTATION || Settings.export_novt) {
    ...
    uint32_t createClusterNumbers(std::vector<std::unique_ptr<SwitchFunction>> &createdFunctions,
      ...
      if (cluster.size() > 5) {
        ...
        for (auto &sf: createdFunctions) {
          if (sf->idents.size() < 4 || sf->idents.size() >= cluster.size() || sf->getIdentSizeIgnoringStdlib() == 0 ||
              (biggest && sf->getIdentSizeIgnoringStdlib() <= biggest->getIdentSizeIgnoringStdlib()))
            continue;
    ...
    void AbiBuilder::optIdentNumbers() {
      ...
      // The JIT needs its own cluster
      std::set<DefClassIdent *> jitCluster;
      for (const auto &ident: idents) {
        if (ident->removed || ident->replacedBy || ident->abstract || ident->in_cluster) continue;
        if (ident->cls->isExternal) {
          collectCluster(jitCluster, ident.get());
        }
      }
      if (!jitCluster.empty()) {
        if (Settings.lld_is_shared) {
          size_t ModuleID = std::hash<std::string>{}(Settings.output_filename);
          jit_min_id = ModuleID & 0xffffffff;
        } else {
          jit_min_id = 0;
        }
        jit_max_id = createClusterNumbers(createdFunctions, jitCluster, jit_min_id) - 1;
        llvm::errs() << "[OPT] Internal ident numbers are " << jit_min_id << " - " << jit_max_id << "\n";
      }
...
PreservedAnalyses DebugPass::run(Module &M, ModuleAnalysisManager &MAM) {
  auto start = std::chrono::high_resolution_clock::now();
  ...
  if (!function.empty()) {
    std::string name2 = fname + "-function.ll";
    raw_fd_ostream stream2(name2, code);
    if (M.getFunction(function)) {
      stream2 << *M.getFunction(function);
    } else {
      stream2 << function << " not found\n";
    }
    stream2.close();
  }

  llvm::outs() << "Writing " << fname << " took " << seconds_since(start) << " seconds\n";
  return PreservedAnalyses::all();
}

PreservedAnalyses DebugPass::run(Function &F, FunctionAnalysisManager &MAM) {
  if (!function.empty()) {
    if (F.hasName() && F.getName().str() == function) {
      auto start = std::chrono::high_resolution_clock::now();
      std::error_code code;
      raw_fd_ostream stream2(fname, code);
      stream2 << F;
      stream2.close();
      llvm::outs() << "Writing " << fname << " took " << seconds_since(start) << " seconds\n";
    }
