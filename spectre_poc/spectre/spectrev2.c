...
void secret_fun(int offset){
    ...
    // printf("secret(%d) (%x) %s\n", offset, (secret[index] >> shift) & (VALUES - 1), secret);
...
// victim function.
void victim(volatile void(**func)(int), size_t offset){
#ifdef MITIGATION_ASM
    // use inline assembly for call and insert some assembly code before
    __asm__ volatile(
         "mov x1, [%[func_ptr]]\n"
         MITIGATION_ASM "\n"
         "blr x1\n"
         :: [func_ptr] "r" (func) : "x1", "x30", "memory"
    );
#else
    (*func)(offset);
...
int setup(char* public_data, char* secret_data){
    ...
    training_offset = strlen(secret) * OFFSETS_PER_BYTE;
